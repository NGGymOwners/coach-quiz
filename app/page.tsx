import Link from "next/link";

export default function Landing() {
  return (
    <main className="flex flex-1 items-center justify-center px-6 py-16">
      <div className="max-w-2xl w-full text-center">
        <p className="font-display text-accent text-lg tracking-[0.3em] mb-6">
          CHEER COACHES&apos; HANDBOOK
        </p>
        <h1 className="font-display text-5xl sm:text-7xl leading-[0.95] mb-6">
          What&apos;s Your
          <br />
          <span className="text-accent">Coach Development</span>
          <br />
          Style?
        </h1>
        <p className="text-foreground-muted text-lg mb-10 max-w-xl mx-auto">
          A 9-question diagnostic for cheer gym owners. Score how well you train
          your coaches across the six pillars that separate good programs from
          great ones — and find out where to focus next.
        </p>
        <Link
          href="/quiz"
          className="inline-block bg-accent hover:bg-accent-hover text-background font-semibold px-10 py-4 rounded-md transition-colors text-lg"
        >
          Start the quiz
        </Link>
        <p className="text-foreground-dim text-sm mt-6">
          ~90 seconds · No email required to start
        </p>
      </div>
    </main>
  );
}
